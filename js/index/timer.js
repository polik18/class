...
const timerEls = {
  ...
  faceToggleBtn: document.getElementById('timer-face-toggle-btn'),
  faceToggleIcon: document.getElementById('timer-face-toggle-icon'),
  faceToggleText: document.getElementById('timer-face-toggle-text'),
  resetBtn: document.getElementById('timer-reset-btn'),
  faceResetBtn: document.getElementById('timer-face-reset-btn'),
...
const setTimerToggleUi = (isRunning) => {
  const primaryBtn = timerEls.toggleBtn;
  const faceBtn = timerEls.faceToggleBtn;
  const iconName = isRunning ? 'pause' : 'play';
  const fullText = isRunning ? '暫停計時' : '開始計時';
  const faceText = isRunning ? '暫停' : '開始';

  primaryBtn.classList.replace(isRunning ? 'bg-indigo-600' : 'bg-amber-500', isRunning ? 'bg-amber-500' : 'bg-indigo-600');
  primaryBtn.classList.replace(isRunning ? 'hover:bg-indigo-500' : 'hover:bg-amber-400', isRunning ? 'hover:bg-amber-400' : 'hover:bg-indigo-500');
  primaryBtn.classList.replace(isRunning ? 'shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'shadow-[0_0_20px_rgba(245,158,11,0.4)]', isRunning ? 'shadow-[0_0_20px_rgba(245,158,11,0.4)]' : 'shadow-[0_0_20px_rgba(99,102,241,0.4)]');
  primaryBtn.classList.replace(isRunning ? 'hover:shadow-[0_0_30px_rgba(99,102,241,0.6)]' : 'hover:shadow-[0_0_30px_rgba(245,158,11,0.6)]', isRunning ? 'hover:shadow-[0_0_30px_rgba(245,158,11,0.6)]' : 'hover:shadow-[0_0_30px_rgba(99,102,241,0.6)]');
  timerEls.toggleIcon.setAttribute('data-lucide', iconName);
  timerEls.toggleText.textContent = fullText;

  if (faceBtn) {
    faceBtn.classList.toggle('is-running', isRunning);
    faceBtn.setAttribute('aria-label', isRunning ? '暫停計時' : '開始計時');
    faceBtn.setAttribute('title', isRunning ? '暫停計時' : '開始計時');
    timerEls.faceToggleIcon?.setAttribute('data-lucide', iconName);
    if (timerEls.faceToggleText) timerEls.faceToggleText.textContent = isRunning ? '點擊暫停' : '點擊開始';
  }
};

const resetTimerToInputs = () => {
  stopTimer();
  applyInputsToTimer();
...
timerEls.toggleBtn.addEventListener('click', toggleTimer);
timerEls.faceToggleBtn?.addEventListener('click', toggleTimer);

timerEls.resetBtn.addEventListener('click', resetTimerToInputs);
timerEls.faceResetBtn?.addEventListener('click', resetTimerToInputs);
