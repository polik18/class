    ...
    window.togglePomoTimer = () => {
        ...
        if (pomoState.isRunning) {
            ...
            if (pomoState.mode === 'focus' && pomoState.remainingSecs === POMO_SECONDS_WORK) playCuckooSound();
            if (pomoState.mode === 'break' && pomoState.remainingSecs === POMO_SECONDS_BREAK) playRelaxSound();
        }
    ...
    const updatePomoDisplay = (refreshIcons = false) => {
        const totalSecs = Math.max(1, pomoState.mode === 'focus' ? POMO_SECONDS_WORK : POMO_SECONDS_BREAK);
        const safeRemaining = Math.max(0, Math.min(pomoState.remainingSecs, totalSecs));
        ...
        shell?.classList.toggle('is-break', pomoState.mode === 'break');
        shell?.classList.toggle('is-running', pomoState.isRunning);

        const badge = document.getElementById('pomo-mode-badge');
        if (badge) {
            badge.textContent = pomoState.mode === 'focus' ? '專注時間' : '休息時間';
            badge.className = 'pomo-badge';
        }

        const subtitle = document.getElementById('pomo-state-subtitle');
        if (subtitle) {
            subtitle.textContent = pomoState.mode === 'focus'
                ? '把注意力收進這顆蕃茄裡，先完成眼前這一段。'
                : '休息不是偷懶，是讓下一輪專注更漂亮。';
        }

        const caption = document.getElementById('pomo-time-caption');
        if (caption) {
            if (pomoState.isRunning) caption.textContent = pomoState.mode === 'focus' ? '專注進行中' : '休息進行中';
            else caption.textContent = safeRemaining === totalSecs ? '準備開始' : '已暫停';
        }

        const cycleLabel = document.getElementById('pomo-cycle-label');
        if (cycleLabel) cycleLabel.textContent = pomoState.mode === 'focus' ? 'Focus' : 'Break';

        const nextLabel = document.getElementById('pomo-next-label');
        if (nextLabel) {
            nextLabel.textContent = pomoState.mode === 'focus'
                ? `下一段會切換成 ${Math.round(POMO_SECONDS_BREAK / 60)} 分鐘休息。`
                : `休息結束後回到 ${Math.round(POMO_SECONDS_WORK / 60)} 分鐘專注。`;
        }

        const focusPill = document.getElementById('pomo-focus-pill');
        if (focusPill) focusPill.textContent = Math.round(POMO_SECONDS_WORK / 60);
        const breakPill = document.getElementById('pomo-break-pill');
        if (breakPill) breakPill.textContent = Math.round(POMO_SECONDS_BREAK / 60);

        const progressRing = document.getElementById('pomo-progress-ring');
        if (progressRing) {
            const ratio = safeRemaining / totalSecs;
            progressRing.style.strokeDasharray = '1000';
            progressRing.style.strokeDashoffset = String(1000 - (1000 * ratio));
        }

        const icon = document.getElementById('pomo-toggle-icon');
        if (icon) icon.setAttribute('data-lucide', pomoState.isRunning ? 'pause' : 'play');

        const toggleText = document.getElementById('pomo-toggle-text');
        if (toggleText) toggleText.textContent = pomoState.isRunning ? '暫停' : '開始';

        const toggleBtn = document.getElementById('pomo-toggle-btn');
        toggleBtn?.classList.toggle('is-running', pomoState.isRunning);
